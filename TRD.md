# TRD — Assurance Harness

> Technical contract for the engine behind the audit. Codename only; see PRD.md, ARCHITECTURE.md, RESEARCH.md, and apatureai/core #130/#134.
...
3. **Fail-closed on data egress.** Runs in the client VPC with the client's keys; only scores/metrics/derived artifacts leave; raw outputs and prompts never egress; zero-retention.
4. **No real model/key/network in tests.** Fixtures + injected ports only.
5. **One-directional reuse.** May import `@engine/eval` (calibration IP) one-directionally; never a reverse dependency, so the deliverable's independence is uncontaminated.

## 2. Pipeline (TRD §; methodology #130)
...
- `Gateway.run({model, taskId, trial, input}) → {output, costUsd, latencyMs}`. Production adapter wires LiteLLM/OpenRouter **inside the client VPC**; the harness never holds a key. Tests inject `StubGateway` (fixtures, may vary output per trial for variance).

### 3.3 Calibrated judge metrics (`metrics.ts`)
- Input: `JudgePrediction[] = {confidence, correct}` where `correct = (judge verdict == human label)`.
- `expectedCalibrationError` (binned, count-weighted gap), `brierScore`, `reliabilityTable`. The quality number ships **with its own error bars** — the differentiator no conflicted vendor reproduces.
